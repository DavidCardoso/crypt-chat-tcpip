...
#	@sa 		https://github.com/davidcardoso-ti/crypt-chat-tcpip/blob/master/client_chat.py
...
# Variáveis
TIMEOUT_S 	= 2.0  				#< TIMEOUT_S 	- tempo de espera para o socket (segundos)
...
print "   Conexão estabelecida com o servidor %s porta %s %s" 						% (GREEN+host_ip+NORMAL, GREEN+str(host_port)+NORMAL, ENDL)
				...
				# Decifra e imprime mensagem no console	
					...
					try:
						...
						if data_readable == "":
							...
							data_readable += "%s   1. Ou tente ajustar o criptossistema para %s." 						% (ENDL, ('SDES' if CRIPT == 'r' else 'RC4') )
							data_readable += "%s   2. Ou envie %s<senha> para mudar a senha diretamente." 				% (ENDL, ALTER_PASS)
							data_readable += "%s   3. Ou envie %s para solicitar a chave pública de outro cliente." 	% (ENDL, GET_PUBKEY)
